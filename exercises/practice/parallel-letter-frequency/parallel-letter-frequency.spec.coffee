PLF = require './parallel-letter-frequency'
...
describe 'Parallel Letter Frequency', ->
  testAsync = (testFn) ->
    (done) ->
      testFn()
        .then -> done()
        .catch (err) -> done.fail?(err) or done(err)

  it 'no texts', testAsync ->
    texts = []
    expected = {}
    PLF.parallelLetterFrequency(texts).then (result) ->
      expect(result).toEqual expected

  xit 'one text with one letter', testAsync ->
    texts = ["a"]
    expected = {"a": 1}
    PLF.parallelLetterFrequency(texts).then (result) ->
      expect(result).toEqual expected

  xit 'one text with multiple letters', testAsync ->
    texts = ["bbcccd"]
    expected = {"b": 2, "c": 3, "d": 1}
    PLF.parallelLetterFrequency(texts).then (result) ->
      expect(result).toEqual expected

  xit 'two texts with one letter', testAsync ->
    texts = ["e", "f"]
    expected = {"e": 1, "f": 1}
    PLF.parallelLetterFrequency(texts).then (result) ->
      expect(result).toEqual expected

  xit 'two texts with multiple letters', testAsync ->
    texts = ["ggh", "hhi"]
    expected = {"g": 2, "h": 3, "i": 1}
    PLF.parallelLetterFrequency(texts).then (result) ->
      expect(result).toEqual expected

  xit 'ignore letter casing', testAsync ->
    texts = ["m", "M"]
    expected = {"m": 2}
    PLF.parallelLetterFrequency(texts).then (result) ->
      expect(result).toEqual expected

  xit 'ignore whitespace', testAsync ->
    texts = ["   ", "\t", "\r\n"]
    expected = {}
    PLF.parallelLetterFrequency(texts).then (result) ->
      expect(result).toEqual expected

  xit 'ignore punctuation', testAsync ->
    texts = ["!", "?", ";", ",", "."]
    expected = {}
    PLF.parallelLetterFrequency(texts).then (result) ->
      expect(result).toEqual expected

  xit 'ignore numbers', testAsync ->
    texts = ["1", "2", "3", "4", "5", "6", "7", "8", "9"]
    expected = {}
    PLF.parallelLetterFrequency(texts).then (result) ->
      expect(result).toEqual expected

  xit 'Unicode letters', testAsync ->
    texts = ["本", "φ", "ほ", "ø"]
...
his worst defect is his perpetual moral obliquity, perpetual - from the days of the Flood to the Schleswig-Holstein period."""
  ...
  xit 'many small texts', testAsync ->
    texts = Array(50).fill("abbccc")
